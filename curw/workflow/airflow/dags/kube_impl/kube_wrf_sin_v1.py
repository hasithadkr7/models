...
dag = DAG(
    ...
    description='Running a single WRF using Google Kubernetes Engine',
...
extract_pod_no_push.spec.containers[0].args = ['-i', '{{ ti.xcom_pull(task_ids=\'gen-run-id\') }}',
                                               '-c', '{{ ti.xcom_pull(task_ids=\'init-config\') }}',
